...
import { useMode, DASHBOARD_MODES, type DashboardMode } from '@/contexts/ModeContext';
import { useClickAway } from '@/hooks/useClickAway';

export default function ModeSelector() {
  const { currentMode, setMode, getCurrentModeConfig } = useMode();
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`group relative flex items-center gap-2.5 px-3 py-2 rounded-xl backdrop-blur-md transition-all duration-300 hover:scale-[1.02] ${
          isOpen 
      ...
      >
        ...
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-medium transition-all duration-300 ${
          currentModeConfig.id === 'mc-assist' 
          ...
          <div className="p-3 space-y-1">
            {DASHBOARD_MODES.map((mode, index) => (
              <button
                ...
                className={`group w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-300 text-left relative overflow-hidden ${
                  currentMode === mode.id 
              ...
              >
                {/* Mode Icon */}
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-medium flex-shrink-0 transition-all duration-300 group-hover:scale-110 ${
                  mode.id === 'mc-assist' 
